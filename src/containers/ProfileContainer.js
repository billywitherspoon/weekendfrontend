...
class ProfileContainer extends Component {
	...
	fetchFavorites = () => {
		fetch(`https://weekendweatherwatcherbackend/api/v1/favorites/user/${this.currentUser.id}`)
			.then((res) => res.json())
			.then((json) => {
				// let allFavorites = [];
	...
	fetchUser = () => {
		fetch(`https://weekendweatherwatcherbackend/api/v1/users/${this.currentUser.id}`)
			.then((res) => res.json())
			.then((json) => {
